...
def test_sqlite_persistencia_disponibilidad_atencion():
    with tempfile.TemporaryDirectory() as temp_dir:
        db_path = Path(temp_dir) / "consultas.db"
        consulta_repo = SQLiteConsultaRepository(str(db_path))
        disponibilidad_repo = SQLiteDisponibilidadAtencionRepository(str(db_path))
...
def test_sqlite_disponibilidad_atencion_repository_close():
    with tempfile.TemporaryDirectory() as temp_dir:
        db_path = Path(temp_dir) / "consultas.db"
        repo = SQLiteDisponibilidadAtencionRepository(str(db_path))
        repo.close()
